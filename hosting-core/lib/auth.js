...
class Auth {
  loadAdminConfig() {
    ...
    if (fs.existsSync(adminPath)) {
      try {
        ...
        console.error('[Auth] Error reading admin.json:', error.message);
      }
    }
    return null;
  }
  ...
  async initAdmin() {
    try {
      ...
      if (!adminConfig) {
        console.log('[Auth] No admin.json found, skipping admin setup');
        return;
      }

      const existingAdmin = await db.findOne('Users', { username: adminConfig.username });
      if (existingAdmin) {
      ...
      console.log(`[Auth] Admin user "${adminConfig.username}" created successfully`);
    } catch (error) {
      console.error('[Auth] Init admin error:', error.message);
    }
  }

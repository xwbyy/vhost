...
app.get('/', async (req, res) => {
  try {
    // Jika user login, tampilkan apps milik user saja
    const userId = req.user ? req.user.id : null;
    const apps = userId ? await deployer.getAllApps(userId) : [];
    const stats = {
      totalApps: apps.length,
      runningApps: apps.filter(a => a.isRunning).length,
      stoppedApps: apps.filter(a => !a.isRunning).length
    };
    // Hanya tampilkan "Aplikasi Terbaru" jika user login
    res.render('index', { stats, recentApps: userId ? apps.slice(-5).reverse() : [] });
  } catch (error) {
...
app.get('/apps', requireAuth, async (req, res) => {
  try {
    // Filter apps by user
    const apps = await deployer.getAllApps(req.user.id);
...
app.post('/api/deploy/github', requireAuth, async (req, res) => {
  try {
    ...
    // Pass userId to deployer
    const app = await deployer.deployFromGitHub(repoUrl, appName, { userId: req.user.id });
...
app.post('/api/deploy/zip', requireAuth, upload.single('zipFile'), async (req, res) => {
  try {
    ...
    // Pass userId to deployer
    const app = await deployer.deployFromZip(req.file.path, appName, { userId: req.user.id });
    res.json({ success: true, app });
  } catch (error) {
    if (req.file) {
      await fs.remove(req.file.path);
